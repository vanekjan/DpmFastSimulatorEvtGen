...
void nt_TMVA_Cuts::Loop()
{
  ...
  if(ReadMode != 0) //ReadMode == 0 is for analysis pre-cuts stored in tmvaCuts.h
  {
    ...
    for(unsigned int j = 0; j < nCentBins-1; j++)
    {
      ...
      if(ReadMode == 1)
      {
        filename << "./TMVA_cuts/analysis/cuts_cent_" << j << ".txt";
      }
      if(ReadMode == 2)
      {
        filename << "./TMVA_cuts/loose/50_percent/cuts_loose_cent_" << j << ".txt";
      }
      if(ReadMode == 3)
      {
        filename << "./TMVA_cuts/tight/50_percent/cuts_tight_cent_" << j << ".txt";
      }
